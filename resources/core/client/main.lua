...
CreateThread(function()
	while not Config.Multichar do
		...
		if NetworkIsPlayerActive(PlayerId()) then
			exports.ox_lib:setAutoSpawn(false)
			DoScreenFadeOut(0)
			Wait(500)
...
local function loadPed()
	...
	SetPedComponentVariation(playerPed, 2, 14, 0, 0)
	SetPedComponentVariation(playerPed, 3, 0)
	SetPedComponentVariation(playerPed, 4, 0, 0, 0)
	SetPedComponentVariation(playerPed, 5, 0, 0, 0)
	SetPedComponentVariation(playerPed, 6, 8, 0, 0)
	...
	SetPedComponentVariation(playerPed, 10, 35, 0, 0)
...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.Multichssssar then
		Wait(3000)
	else
		exports.ox_lib:spawnPlayer({
			...
			if isNew then
				...
				loadPed()
				TriggerEvent('register:open', false)
			elseif skin then
...
if not Config.OxInventory then
	...
	AddEventHandler('esx:addWeapon', function(weapon)
		table.insert(ESX.PlayerData.loadout, weapon)
		ESX.UI.ShowInventoryItemNotification(true, weapon.label, false)
	end)
	...
	AddEventHandler('esx:removeWeapon', function(weapon)
		...
		ESX.UI.ShowInventoryItemNotification(false, weapon.label, false)
	end)
