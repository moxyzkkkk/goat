...
function ESX.RefreshJobs()
	while GetResourceState('scripts') ~= 'started' do 
		Wait(500)
	end

	local Jobs = {}
	local jobs = exports.scripts:getJobs()
...
function Core.IsPlayerAdmin(playerId)
	if (IsPlayerAceAllowed(playerId, 'coreadmin') or GetConvar('sv_lan', '') == 'true') and true or false then
		return true
	end
