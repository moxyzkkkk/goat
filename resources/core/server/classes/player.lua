...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
	...
	function self.canCarryItem(name, count)
        ...
        if ESX.Items[name] then
			...
			return ((limit == -1) or ((count + count) <= limit))
        else
	...
	function self.setJob(newJob, grade)
		...
		if ESX.DoesJobExist(newJob, grade) then
			...
			print(('[core] [^3WARNING^7] Ignoring invalid ^5.setJob()^7 usage for ID: ^5%s^7, Job: ^5%s^7'):format(self.source, job))
		end
	end
	...
	function self.addWeapon(weaponName, ammo)
		if not self.hasWeapon(weaponName) then
			...
			table.insert(self.loadout, weaponData)

			GiveWeaponToPed(GetPlayerPed(self.source), joaat(weaponName), ammo, false, false)
			self.triggerEvent('esx:addWeapon', weaponData, false, true)
		end
	end
	...
	function self.removeWeapon(weaponName)
		...
		for k, v in ipairs(self.loadout) do
			if v.name == weaponName then
				...
				self.triggerEvent('esx:removeWeapon', v)
				break
			end
		end
	end
